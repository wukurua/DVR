...
ForgetPwdWindow::ForgetPwdWindow(Dialog *parent) : Dialog(parent)
{
...
void ForgetPwdWindow::commit()
{
    ...
    {
        if(this->etName->text().length()<8)
            this->lblNameMsg->setText("账号不可小于8位");
    }
    ...
    else
    {
        if(this->etPwd->text().length()<6||this->etPwd->text().length()>16)
            this->lblPwdMsg->setText("密码长度必须在6~16位之间");
        else
    ...
    else
    {
        ...
        {
            int ret=userController->changePwdByNameAndTel(etName->text(),etPwd->text(),etTelePhone->text());
            if(ret!=0)
                this->lblTelephoneMsg->setText("更改失败,不存在此电话号码");
            else
...
void ForgetPwdWindow::jumpToLoginWin()
{
    emit showLoginWinSignal("","");
    this->hide();
}

void ForgetPwdWindow::refreshCode()
{
    lblVerficationCode->slt_reflushVerification();
}

void ForgetPwdWindow::setNameSlot(QString name)
{
    this->etName->setText(name);
}

void ForgetPwdWindow::initWidgets()
{
    lblWrap=new QLabel("",this);
    lblTitle=new QLabel("Reset Your Password",this);
...
void ForgetPwdWindow::initLayout()
{
    ...
    lblTelephoneImg->setGeometry(362,442,230,40);
...
void ForgetPwdWindow::initConnect()
{
    connect(btnOK,SIGNAL(clicked(bool)),this,SLOT(commit()));
...
bool ForgetPwdWindow::eventFilter(QObject *obj, QEvent *event)
{
